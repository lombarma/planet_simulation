...
class Planet:
    ...
    def __init__(self, x, y, radius, color, mass, name_planet):
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
    ...
    def draw(self, win):
        ...
        if not self.sun:
            distance_text = FONT.render(f"{round(self.distance_to_sun/1000, 1)}km, {self.name_planet}", 1, WHITE)
            win.blit(distance_text, (x - distance_text.get_width()/2, y-distance_text.get_height()/2))

    def attraction(self, other):  # other is and other object that we're calculating the force of attraction
